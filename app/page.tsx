...
export default function Home() {
  ...
  // Fungsi otomatis untuk handle klik navigasi khusus kontak/sosmed
  const handleNavigation = (section: string) => {
    if (section === 'whatsapp') {
      // Ganti nomor di bawah ini dengan nomor WhatsApp kamu (gunakan format kode negara, misal 628xxx)
      window.open('https://wa.me/6281234567890', '_blank')
      return
    }
    if (section === 'email') {
      window.location.href = 'mailto:inimuhammad777@gmail.com'
      return
    }
        ...
        <div className="flex flex-col lg:flex-row gap-3 sm:gap-4 md:gap-6">
          ...
          <main className="flex-1 bg-card rounded-xl md:rounded-2xl border border-border overflow-hidden">
            ...
            <nav className="flex gap-1 sm:gap-2 md:gap-4 p-3 sm:p-4 md:p-6 border-b border-border overflow-x-auto scrollbar-hide">
              {['about', 'resume', 'portfolio', 'blog', 'contact', 'whatsapp', 'email'].map((section) => (
                <button
                  key={section}
                  ...
                  } ${['whatsapp', 'email'].includes(section) ? 'border border-dashed border-primary/40 text-primary hover:bg-primary/5' : ''}`}
                >
                  {section === 'whatsapp' ? '💬 WhatsApp' : section === 'email' ? '✉️ Email' : section}
                </button>
              ))}
            </nav>
